...
AnaTtresNoTrigger::AnaTtresNoTrigger(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  ...
  m_hSvc.create1D("wpt_mtt_2000", "; W p_{T} [GeV] ; Events", 20, 0, 1000);
  m_hSvc.create1D("wpt_mtt_1000", "; W p_{T} [GeV] ; Events", 20, 0, 1000);
...
AnaTtresNoTrigger::~AnaTtresNoTrigger() {
}

void AnaTtresNoTrigger::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  ...
  if (m_boosted && (is2016run && (evt.passes("bejets_2016") || evt.passes("bmujets_notrigger")))
    ...
    //MET = evt.met().Perp()*1e-3;
    //WPt = sqrt(pow(MET,2) + pow(l.Perp()*1e-3,2));
    if(mtt*1e-3 > 2000) h->h1D("wpt_mtt_2000", "", suffix)->Fill(WPt, weight); 
    if(mtt*1e-3 > 1000) h->h1D("wpt_mtt_1000", "", suffix)->Fill(WPt, weight); 
