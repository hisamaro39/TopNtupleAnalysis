...
AnaTtresTrigMetOnly::AnaTtresTrigMetOnly(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  : AnalysisTrig(filename, systList), m_electron(electron), m_boosted(boosted),
  ...
  m_hSvc.create1D("wpt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("wpt_highMtt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("wpt_lowMtt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("trueWpt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("trueWpt_highMtt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("trueWpt_lowMtt", "; W p_{T} [GeV] ; Events", 50, 0, 1000);
  m_hSvc.create1D("trueWeta", "; W #eta ; Events", 25, -2.5, 2.5);
  m_hSvc.create1D("trueWeta_highMtt", "; W #eta ; Events", 25, -2.5, 2.5);
  m_hSvc.create1D("trueWeta_lowMtt", "; W #eta ; Events", 25, -2.5, 2.5);
...
AnaTtresTrigMetOnly::~AnaTtresTrigMetOnly() {
}

void AnaTtresTrigMetOnly::run(const Event &evt, double weight, const std::string &s, int is2016run, int trthr) {
  ...
  if (m_boosted){
    if (!(evt.passes("bmujets_notrigger") || evt.passes("bejets_2016"))) 
      return;
  }
  ...
  //std::cout << "pass selection cut!" << std::endl;
  ...
  if (m_electron) {
    ...
    //std::cout << "wpt/thr=" << WPt << "/" << trthr << std::endl;
    //if(WPt<trthr) return;
  ...
  if (m_boosted && (is2016run && (evt.passes("bejets_2016") || evt.passes("bmujets_2016") || evt.passes("bmujetsmet110")))
      || (!is2016run && (evt.passes("bejets_2015") || evt.passes("bmujets_2015") || evt.passes("bmujetsmet110")))) {
    ...
    std::cout << "mtt=" << mtt << std::endl;
    if(mtt*1e-3>2000) h->h1D("wpt_highMtt", "", suffix)->Fill(WPt, weight); 
    else h->h1D("wpt_lowMtt", "", suffix)->Fill(WPt, weight); 
    ...
    h->h1D("trueWeta", "", suffix)->Fill(leptonicW.Eta(), weight); 
    h->h1D("trueWpt", "", suffix)->Fill(leptonicW.Perp()*1e-3, weight); 
    if(mtt*1e-3>2000) {
      h->h1D("trueWeta_highMtt", "", suffix)->Fill(leptonicW.Eta(), weight); 
      h->h1D("trueWpt_highMtt", "", suffix)->Fill(leptonicW.Perp()*1e-3, weight); 
    }
    else {
      h->h1D("trueWeta_lowMtt", "", suffix)->Fill(leptonicW.Eta(), weight); 
      h->h1D("trueWpt_lowMtt", "", suffix)->Fill(leptonicW.Perp()*1e-3, weight); 
    }
    ...
    if ((((is2016run && evt.passes("bmujets_2016")) || (!is2016run && evt.passes("bmujets_2015"))) || evt.passes("bmujetsmet110")) 
        && m_boosted && !m_electron) _tree_cat = 1;
    _tree_syst = s;
    h->m_tree->Fill();
