...
AnaTtresTrigLjetOnly::AnaTtresTrigLjetOnly(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
...
AnaTtresTrigLjetOnly::~AnaTtresTrigLjetOnly() {
}

void AnaTtresTrigLjetOnly::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  ...
  if (!m_boosted){
    if (!(evt.passes("rejets_2016") || evt.passes("rmujets_2016") || evt.passes("rmujetsmet90")))
      return;
  }
  ...
  if (m_electron) {
    ...
    //MET triggers
    trig4 = evt.trigger("HLT_j420_a10_lcw_sub_L1J100");
    
    bool trig_muon = trig2 || trig3;
    bool trig_ljet = trig4;
    
    size_t temp_goodljet_idx = 0;
    for (; temp_goodljet_idx < evt.largeJet().size(); ++temp_goodljet_idx) {
      if (evt.largeJet()[temp_goodljet_idx].good())
        break;
    }
    const TLorentzVector &temp_lj = evt.largeJet()[temp_goodljet_idx].mom();
    
    if(temp_lj.Perp()*1e-3<500) return;
  ...
  if (m_boosted && (is2016run && (evt.passes("bejets_2016") || evt.passes("bmujets_2016") || evt.passes("bmujetsljet420")))
      || (!is2016run && (evt.passes("bejets_2015") || evt.passes("bmujets_2015") || evt.passes("bmujetsljet420")))) {
    ...
    if ((((is2016run && evt.passes("bmujets_2016")) || (!is2016run && evt.passes("bmujets_2015"))) || evt.passes("bmujetsljet420")) 
        && m_boosted && !m_electron) _tree_cat = 1;
    _tree_syst = s;
    h->m_tree->Fill();
    
  }else if (!m_boosted && ((is2016run && evt.passes("rejets_2016")) || ((is2016run && evt.passes("rmujets_2016")) || evt.passes("rmujetsljet420")))) {
    ...
    if (is2016run && (evt.passes("rmujets_2016") || evt.passes("rmujetsmet90")) && !m_boosted && !m_electron) _tree_cat = 3;
    h->m_tree->Fill();
  }
}
