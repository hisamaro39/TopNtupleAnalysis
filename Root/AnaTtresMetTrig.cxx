...
AnaTtresMetTrig::AnaTtresMetTrig(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  ...
  m_hSvc.create1DVar("MET_passMet_pufit120", "; missing E_{T} [GeV]; Events", varBinN1, varBin1);
  m_hSvc.create1DVar("MET_passMet_L160", "; missing E_{T} [GeV]; Events", varBinN1, varBin1);
  m_hSvc.create1D("MET_phi", "; missing E_{T} #phi [rd] ; Events", 32, -3.2, 3.2);
  m_hSvc.create1D("dphi_met_lepton", "; #Delta#phi_{MET,#mu} [rd] ; Events", 32, 0, 3.2);
  m_hSvc.create1D("dif_MET_truthMET", "; MET - truthMET (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_MET_truthMETnomuon", "; MET - truthMETnomuon (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_MET_truthMETmuon", "; MET - truthMETmuon (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_wpt_truthMET", "; wpt - truthMET (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_wpt_truthMETnomuon", "; wpt - truthMETnomuon (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_wpt_truthMETmuon", "; wpt - truthMETmuon (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_muonpt_truthMETmuon", "; muonpt - truthMETmuon (GeV); Events", 100,-100,100);
  m_hSvc.create1D("dif_muonpt_MET", "; muonpt - MET (GeV); Events", 100,-100,100);
  m_hSvc.create1DVar("truthMET", ";truth MET [GeV]; Events", varBinN1, varBin1);
  m_hSvc.create1DVar("truthMET_passMet_pufit120", "; truth MET [GeV]; Events", varBinN1, varBin1);
  m_hSvc.create1DVar("truthMET_passMet_L160", "; truth MET [GeV]; Events", varBinN1, varBin1);
  
  m_hSvc.create1D("truthMETnomuon", ";truth MET(no #mu) [GeV]; Events", 25,0,500);
  m_hSvc.create1D("truthMETnomuon_passMet_pufit120", "; truth MET(no #mu) [GeV]; Events", 25,0,500);
  m_hSvc.create1D("truthMETnomuon_passMet_L160", "; truth MET(no #mu) [GeV]; Events", 25,0,500);
  m_hSvc.create1D("wpt", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_L160", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_pufit100", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_pufit110", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_pufit120", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_mht120_L160", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passMet_mht120_L150", "; W p_{T} [GeV] ; Events", 25, 0, 500);
  m_hSvc.create1D("wpt_passLJet", "; W p_{T} [GeV] ; Events", 20, 0, 1000);
  m_hSvc.create2D("wpt_vs_dphi_lep_met", "; wpt (GeV); #Delta#phi_{MET,#mu}", 30,0,300,32,0,3.2);
  m_hSvc.create2D("truth_met_nomuon_vs_dphi_lep_met", "; truth MET(no #mu) (GeV); #Delta#phi_{MET,#mu}", 30,0,300,32,0,3.2);
  ...
  m_hSvc.create2D("lepEta_wpt", ";lepton #eta; W p_{T} [GeV]",20, -2.5, 2.5, 100, 0, 1000);
...
//    m_hSvc.create1DP(name, "; #Delta R; Events", 31, 0.2, 0.5, 0., 1.);
  ...
  if (m_boosted) {
    ...
    m_hSvc.create1DVar("largeJetPt", "; large jet p_{T} [GeV] ; Events", varBinN3, varBin3);
    m_hSvc.create1DVar("largeJetPt_passMet", "; large jet p_{T} [GeV] ; Events", varBinN3, varBin3);
    m_hSvc.create1DVar("largeJetPt_passLJet", "; large jet p_{T} [GeV] ; Events", varBinN3, varBin3);
...
AnaTtresMetTrig::~AnaTtresMetTrig() {
}

void AnaTtresMetTrig::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  // check channel
  //
  float truth_top_eta = evt.MC_th().Eta();
  float truth_top_phi = evt.MC_th().Phi();
  //std::cout << "truth top eta/phi=" << truth_top_eta << "/" << truth_top_phi << std::endl;
  char name[200];
  ...
  std::cout << "pass lepton selection" << std::endl;

  //std::cout << "ljet size=" << evt.largeJet().size() << std::endl;
  //if(evt.largeJet().size()==0) return;
  //const TLorentzVector &lj_tmp = evt.largeJet()[0].mom();
  //if(lj_tmp.Perp()*1e-3<200) return;
  ...
  float cos_dphi_met_lepton = cos(evt.met().DeltaPhi(l));
  float WPt = sqrt(pow(MET,2) + pow(l.Perp()*1e-3,2)+2*MET*l.Perp()*1e-3*cos_dphi_met_lepton);

  std::string suffix = s;
  //if (s=="_Loose")	suffix = "";
  
  h->h1D("dphi_met_lepton", "", suffix)->Fill(evt.met().DeltaPhi(l),weight); 
  ...
  h->h2D("lepEta_wpt","",suffix)->Fill(l.Eta(), WPt, weight);
  ...
  //std::cout << "lepton eta=" << l.Eta() << std::endl;
  ...
  if(evt.trigger("HLT_xe120_pufit")) h->h1D("MET_passMet_pufit120", "", suffix)->Fill(MET,weight); 
  if(evt.trigger("L1_XE60")) h->h1D("MET_passMet_L160", "", suffix)->Fill(MET,weight); 
  float truth_met = evt.MC_met()*0.001;
  float truth_met_nomuon = evt.MC_met_nomuon()*0.001;
  float truth_met_muon = evt.MC_met_muon()*0.001;
  float dif_MET_truthMET = MET - truth_met;
  float dif_MET_truthMETnomuon = MET - truth_met_nomuon;
  float dif_MET_truthMETmuon = MET - truth_met_muon;
  float dif_WPt_truthMET = WPt - truth_met;
  float dif_WPt_truthMETnomuon = WPt - truth_met_nomuon;
  float dif_WPt_truthMETmuon = WPt - truth_met_muon;
  float dif_muonpt_truthMETmuon = l.Perp()*1e-3 - truth_met_muon;
  float dif_muonpt_MET = l.Perp()*1e-3 - MET;
  TLorentzVector trans_vec_met,trans_vec_lep;
  trans_vec_met.SetPtEtaPhiM(MET,0,evt.met().Phi(),0);
  trans_vec_lep.SetPtEtaPhiM(l.Perp()*1e-3,0,l.Phi(),0);
  float tl_met_nomuon = (trans_vec_met+trans_vec_lep).Pt();
  //if(dif_WPt_truthMETnomuon>-10 && dif_WPt_truthMETnomuon<10){
    std::cout << "tl met_nomuon=" << tl_met_nomuon << std::endl;
    std::cout << "truth met/nomuon/muon=" << truth_met << "/" << truth_met_nomuon << "/" << truth_met_muon << std::endl;
    std::cout << "phi met/lepton=" << evt.met().Phi() << "/" << l.Phi() << std::endl;
    std::cout << "met/leppt/cos(dphi)/wpt/muonpt=" 
      << MET << "/" << l.Perp()*1e-3 << "/" << cos_dphi_met_lepton << "/" << WPt << "/" << l.Perp()*1e-3 << std::endl;
  //}
  h->h1D("dif_MET_truthMET", "", suffix)->Fill(dif_MET_truthMET, weight);
  h->h1D("dif_MET_truthMETnomuon", "", suffix)->Fill(dif_MET_truthMETnomuon, weight);
  h->h1D("dif_MET_truthMETmuon", "", suffix)->Fill(dif_MET_truthMETmuon, weight);
  h->h1D("dif_wpt_truthMET", "", suffix)->Fill(dif_WPt_truthMET, weight);
  h->h1D("dif_wpt_truthMETnomuon", "", suffix)->Fill(dif_WPt_truthMETnomuon, weight);
  h->h1D("dif_wpt_truthMETmuon", "", suffix)->Fill(dif_WPt_truthMETmuon, weight);
  h->h1D("dif_muonpt_truthMETmuon", "", suffix)->Fill(dif_muonpt_truthMETmuon, weight);
  h->h1D("dif_muonpt_MET", "", suffix)->Fill(dif_muonpt_MET, weight);
  h->h1D("truthMET", "", suffix)->Fill(truth_met, weight);
  if(evt.trigger("HLT_xe120_pufit")) h->h1D("truthMET_passMet_pufit120", "", suffix)->Fill(truth_met, weight); 
  if(evt.trigger("L1_XE60")) h->h1D("truthMET_passMet_L160", "", suffix)->Fill(truth_met, weight); 
  h->h1D("truthMETnomuon", "", suffix)->Fill(truth_met_nomuon, weight);
  if(evt.trigger("HLT_xe120_pufit")) h->h1D("truthMETnomuon_passMet_pufit120", "", suffix)->Fill(truth_met_nomuon, weight); 
  if(evt.trigger("L1_XE60")) h->h1D("truthMETnomuon_passMet_L160", "", suffix)->Fill(truth_met_nomuon, weight); 
 
  //transverse W mass  
  h->h1D("mwt", "", suffix)->Fill(sqrt(2. * l.Perp() * evt.met().Perp() * (1. - cos(l.Phi() - evt.met().Phi())))*1e-3, weight); 

  h->h1D("wpt", "", suffix)->Fill(WPt, weight); 
  h->h2D("wpt_vs_dphi_lep_met", "", suffix)->Fill(WPt, evt.met().DeltaPhi(l),weight); 
  h->h2D("truth_met_nomuon_vs_dphi_lep_met", "", suffix)->Fill(truth_met_nomuon, evt.met().DeltaPhi(l),weight); 
  if(evt.trigger("L1_XE60")) h->h1D("wpt_passMet_L160", "", suffix)->Fill(WPt, weight); 
  if(evt.trigger("HLT_xe100_pufit_L1XE60")) h->h1D("wpt_passMet_pufit100", "", suffix)->Fill(WPt, weight); 
  if(evt.trigger("HLT_xe110_pufit_L1XE60")) h->h1D("wpt_passMet_pufit110", "", suffix)->Fill(WPt, weight); 
  if(evt.trigger("HLT_xe120_pufit")) h->h1D("wpt_passMet_pufit120", "", suffix)->Fill(WPt, weight); 
  if(evt.trigger("HLT_xe120_mht_L1XE50")) h->h1D("wpt_passMet_mht120_L150", "", suffix)->Fill(WPt, weight); 
  if(evt.trigger("HLT_xe120_mht")) h->h1D("wpt_passMet_mht120_L160", "", suffix)->Fill(WPt, weight); 
  ...
  if (m_boosted && (is2016run && (evt.passes("bejets_2016") || evt.passes("bmujets_2016")))
      || (!is2016run && (evt.passes("bejets_2015") || evt.passes("bmujets_2015")))) {
    ...
    */
    ...
    _tree_weight = weight;
    _tree_cat = -1;
    if (((is2016run && evt.passes("bejets_2016")) || (!is2016run && evt.passes("bejets_2015")))  && m_boosted && m_electron) _tree_cat = 0;
    if (((is2016run && evt.passes("bmujets_2016") || !is2016run && evt.passes("bmujets_2015"))) && m_boosted && !m_electron) _tree_cat = 1;
    _tree_syst = s;
    h->m_tree->Fill();
