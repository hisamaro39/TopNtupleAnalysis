...
namespace TuDoAtlas {
  ...
  double calc_delta_r(double eta1, double eta2, double phi1, double phi2){
  ...
  double calc_dl1_weight(double pb, double pc, double pu, int mode){
    float m_fraction=-1;
    float weight = -1;
    if(mode==0) m_fraction = 0.08;//dl1 or dl1mu 
    else if(mode==1) m_fraction = 0.03;//dl1rnn
    else {
      std::cout << "you only use mode 0 or 1" << std::endl;
      return weight;
    }
