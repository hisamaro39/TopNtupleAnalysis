...
AnaTtres21_electron::AnaTtres21_electron(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  ...
  m_neutrinoBuilder("MeV"), m_chi2("MeV") {
    ...
    for(int ch=0;ch<6;ch++){
      ...
      for(int c=0;c<6;c++){
        m_hSvc.create1D(Form("muonPt_cut%d_%s",c+1,channel[ch].c_str()), "; muon p_{T} [GeV];", 50,0,500);
        ...
        m_hSvc.create1D(Form("electronPt_cut%d_%s",c+1,channel[ch].c_str()), "; electron p_{T} [GeV];", 50,0,500);
...
AnaTtres21_electron::~AnaTtres21_electron() {
}

void AnaTtres21_electron::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  ...
  //pre trigger
  bool passtrigel = evt.trigger("HLT_e26_lhtight_nod0_ivarloose") || evt.trigger("HLT_e60_lhmedium_nod0") || evt.trigger("HLT_e140_lhloose_nod0"); 

  //pre electron
  //std::cout << "Number of muon is " << evt.muon().size() << std::endl;
  int pre_numElectron=0;
  int pre_lel_id=-1;
  float pre_lel_pt=0.;
  for(int ie=0;ie<evt.electron().size();ie++){
    TLorentzVector el = evt.electron()[ie].mom();
    if(el.Perp()>pre_lel_pt){
      pre_lel_id=ie;
      pre_lel_pt=el.Perp();
    }
  }
  bool hasElectron = (pre_lel_id!=-1)? true : false;
  ...
  if(hasElectron){
    //MWT
    TLorentzVector l;
    l.SetPtEtaPhiM(evt.muon()[pre_lel_id].mom().Perp(),evt.muon()[pre_lel_id].mom().Eta(),evt.muon()[pre_lel_id].mom().Phi(),105.6);
  ...
  int leadmu_id=-1;
  float leadmu_pt=0.;
  bool hasElectronCloseToJet=false;
  for(int im=0;im<evt.nocalibmu().size();im++){
    ...
    if(closejl_deltaR<1.5) hasElectronCloseToJet=true;
  ...
  bool isBoost = hasHighPtLargeJet && hasElectronCloseToJet;
  ...
  //bool pass_trigger = pass_mu26_ivarmedium || pass_mu50;
  if(!passtrig_el1 && !passtrig_el2 && !passtrig_el3) return;
  ...
  int nElectron=0,nElectron25=0,electron_id=-1;
  for(int ie=0;ie<evt.electron().size();ie++){
  ...
  //bool passtrig_el2 = evt.trigger("HLT_e60_lhmedium_nod0"); 
  //bool passtrig_el3 = evt.trigger("HLT_e140_lhloose_nod0"); 
  bool trig1 = evt.electron()[electron_id].HLT_e26_lhtight_nod0_ivarloose();
  bool trig2 = evt.electron()[electron_id].HLT_e60_lhmedium_nod0();
  bool trig3 = evt.electron()[electron_id].HLT_e140_lhloose_nod0();
  ...
  //MET & MET+MWT 
  TLorentzVector l = evt.electron()[electron_id].mom();
  float mWt = sqrt(2. * l.Perp() * evt.met().Perp() * (1. - cos(evt.met().DeltaPhi(l)) ))*1e-3;
  float MET = evt.met().Perp()*1e-3;
  float MET_plus_mWt = mWt + MET;
  ...
  //std::cout << "Number of large jets is " << evt.largeJet().size() << std::endl;
  //Angular cut
  ...
  //No selection track jet
  int numTrackJet=0,nBtaggedTrackJet=0;
  for (size_t jidx = 0; jidx < evt.nocalibtjet().size(); ++jidx){
    TLorentzVector tj = evt.nocalibtjet()[jidx].mom();
    if(tj.Perp()*1e-3<10) continue;
    if(fabs(tj.Eta())>2.4) continue;
    ...
    if(closejmu_deltaR<0.2) continue;
    numTrackJet++;
    if(evt.nocalibtjet()[jidx].btag_mv2c10_70_trk()) nBtaggedTrackJet++;
  }

  if(nBtaggedTrackJet==0) return;
  h->h1D("cutFlow_All", "", suffix)->Fill(11);
  h->h1D(Form("cutFlow_%s",channel.c_str()), "", suffix)->Fill(11);
