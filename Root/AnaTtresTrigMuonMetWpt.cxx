...
AnaTtresTrigMuonMetWpt::AnaTtresTrigMuonMetWpt(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
...
AnaTtresTrigMuonMetWpt::~AnaTtresTrigMuonMetWpt() {
}

void AnaTtresTrigMuonMetWpt::run(const Event &evt, double weight, const std::string &s, int is2016run, int trthr) {
  ...
  if (m_electron) {
    ...
    WPt = sqrt(pow(MET,2) + pow(l.Perp()*1e-3,2)+2*MET*l.Perp()*1e-3*cos_dphi_met_lepton);
    ...
    //trig4 = evt.trigger("HLT_xe120_mht");
    
    bool trig_muon = trig2 || trig3 ;
    bool trig_met = trig4;
    
    if(WPt>trthr){//use MET trigger
      //std::cout << "use met trigger!" << std::endl;
      //std::cout << "met trigger=" << trig_met << std::endl;
      if(!trig_met) return;
    }
    else {//use muon trigger
      //std::cout << "use muon trigger!" << std::endl;
