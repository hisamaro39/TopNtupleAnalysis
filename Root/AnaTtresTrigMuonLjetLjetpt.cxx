...
AnaTtresTrigMuonLjetLjetpt::AnaTtresTrigMuonLjetLjetpt(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
...
AnaTtresTrigMuonLjetLjetpt::~AnaTtresTrigMuonLjetLjetpt() {
}

void AnaTtresTrigMuonLjetLjetpt::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  ...
  if (m_electron) {
    ...
    if(temp_lj.Perp()*1e-3>500){//use large jet trigger
      if(!trig_ljet) return;
    }
    else {//use muon trigger
      if(!trig_muon) return;
    }
  ...
  if (m_boosted && ((evt.passes("bejets_2016") || evt.passes("bmujets_notrigger") ))) {
    ...
    if ((((is2016run && evt.passes("bmujets_notrigger")) || (!is2016run && evt.passes("bmujets_2015")))) 
