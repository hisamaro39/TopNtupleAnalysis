...
float MMUtils::getMMweights(const Event &evt, const int runMM_StatErr, const bool isElectron, const bool isBoosted, const unsigned int runNumber) {
  std::cout << "MMUtils::getMMweights" << std::endl;
  std::cout << "runMM_StatErr/isElectron/isBoosted/runNumber=" << runMM_StatErr << "/" << isElectron << "/" << isBoosted << "/" << runNumber << std::endl;
   
   bool isTight;
   float d0sig;
   float topoetcone;
   std::cout << "size of muon/electron=" << evt.electron().size() << "/" << evt.muon().size() << std::endl;
   ...
   std::cout << "lepPt/closejl_DR/absEta/codDPhi/mWt/topoetcone=" 
     << lepPt << "/" << closejl_DR << "/" << absEta << "/" << cosDPhi << "/" << mWt << "/" << topoetcone << std::endl;
...
//    }//isBoosted	
   ...
   std::cout << "rate real/fake=" << realRate << "/" << fakeRate << std::endl;
   if (isTight){
