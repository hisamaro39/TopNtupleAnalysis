...
AnaTtresNoIso::AnaTtresNoIso(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  ...
  m_neutrinoBuilder("MeV"), m_chi2("MeV") {
    ...
    for(int ch=0;ch<5;ch++) {
      ...
      m_hSvc.create1DVar(Form("mttChange%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
      m_hSvc.create2D(Form("mtt_vs_mlt%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", 60,0,6000,60,0,600);
      m_hSvc.create2D(Form("mtt_vs_mht%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", 60,0,6000,60,0,600);
      m_hSvc.create1D(Form("mlt%s",channel[ch].c_str()), "; mass of leptonic side top [GeV]; Events", 60,0,600);
      m_hSvc.create1D(Form("mltChange%s",channel[ch].c_str()), "; mass of leptonic side top [GeV]; Events", 60,0,600);
      m_hSvc.create1D(Form("mltClosestLjet%s",channel[ch].c_str()), "; mass of leptonic side top [GeV]; Events", 60,0,600);
      m_hSvc.create1D(Form("mltClosestLjetLepton%s",channel[ch].c_str()), "; mass of leptonic side top [GeV]; Events", 60,0,600);
      m_hSvc.create1D(Form("mht%s",channel[ch].c_str()), "; mass of hadronic side top [GeV]; Events", 60,0,600);
      m_hSvc.create1DVar(Form("mttNoNeutrino%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
      m_hSvc.create1DVar(Form("mttClosestLjet%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
      m_hSvc.create1DVar(Form("mttClosestLjetLepton%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
      m_hSvc.create1D(Form("closestLjetGood%s",channel[ch].c_str()), "; good; Events", 3,0,3);
      m_hSvc.create1D(Form("closestLjetGood50%s",channel[ch].c_str()), "; good; Events", 3,0,3);
      m_hSvc.create1D(Form("closestLjetMass%s",channel[ch].c_str()), "; closest large jet mass [GeV]; Events", 60,0,600);
      m_hSvc.create1D(Form("closestSmallJetMass%s",channel[ch].c_str()), "; closest small jet mass [GeV]; Events", 60,0,600);
      m_hSvc.create2D(Form("mtt_vs_closestSmallJetMass%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", 60,0,6000,60,0,600);
      m_hSvc.create2D(Form("mtt_vs_drMuNu%s",channel[ch].c_str()), "; mass of the top-antitop system [GeV]; #DeltaR_{#mu,#nu}", 60,0,6000,60,0,60);
      m_hSvc.create1D(Form("drMuNu%s",channel[ch].c_str()), "; #DeltaR_{#mu,#nu}; Events", 60,0,6);
      for(int t=0;t<3;t++){
        ...
        m_hSvc.create1DVar(Form("mttNoNeutrino%s_%s",type[t].c_str(),channel[ch].c_str()), "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
      }
    }
    ...
    m_hSvc.create1DVar("mttChangeAll", "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
...
AnaTtresNoIso::~AnaTtresNoIso() {
}

void AnaTtresNoIso::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  //std::cout << "AnaTtresNoIso::run" << std::endl;
  ...
  if(isLeptonicT){
    ...
    TVector3 vec_W_from_t,vec_MC_Wdecay2_from_t;
    vec_W_from_t.SetPtEtaPhi(W_from_t.Perp(),W_from_t.Eta(),W_from_t.Phi());
    vec_MC_Wdecay2_from_t.SetPtEtaPhi(evt.MC_Wdecay2_from_t().Perp(),evt.MC_Wdecay2_from_t().Eta(),evt.MC_Wdecay2_from_t().Phi());
    float dphi_muW = evt.MC_Wdecay2_from_t().DeltaPhi(W_from_t); 
    float angle_muW = vec_MC_Wdecay2_from_t.Angle(vec_W_from_t); 
    std::cout << "mu W dphi/angle=" << dphi_muW << "/" << angle_muW << std::endl;
    std::cout << "muon pt/eta/phi=" << evt.MC_Wdecay2_from_t().Perp() << "/" << evt.MC_Wdecay2_from_t().Eta() << "/" << evt.MC_Wdecay2_from_t().Phi() << std::endl;
    float mupt_from_W = evt.MC_Wdecay2_from_t().P() * sin(angle_muW);
    ...
    for(int t=0;t<evt.truth().size();t++){
      ...
      TVector3 vec_truemu,vec_b_from_t;
      vec_truemu.SetPtEtaPhi(pt,eta,phi);
      vec_b_from_t.SetPtEtaPhi(b_pt,b_eta,b_phi);
      ...
      float angle_mub = vec_truemu.Angle(vec_b_from_t);
      std::cout << "mub dphi/angle=" << dphi_mub << "/" << angle_mub << std::endl;
      float mupt_from_b = truemu.P() * sin(angle_mub);
  ...
  for(int m=0;m<evt.inimu().size();m++){
    ...
    if(min_truth_id!=-1){
      ...
      TVector3 vec_initial_muon, vec_closest_jet;
      vec_initial_muon.SetPtEtaPhi(evt.inimu()[m].pt(),evt.inimu()[m].eta(),evt.inimu()[m].phi());
      vec_closest_jet.SetPtEtaPhi(closest_jet.Perp(),closest_jet.Eta(),closest_jet.Phi());
      float dphi_jet_muon = initial_muon.DeltaPhi(closest_jet); 
      float angle_jet_muon = vec_initial_muon.Angle(vec_closest_jet); 
      std::cout << "jet_muon dphi/angle=" << dphi_jet_muon << "/" << angle_jet_muon << std::endl;
      float initial_muon_p = initial_muon.P();
      float muon_pt_to_jet = fabs(initial_muon_p*sin(angle_jet_muon));
      ...
      TVector3 vec_W_tmp;
      vec_W_tmp.SetPtEtaPhi(W_tmp.Perp(),W_tmp.Eta(),W_tmp.Phi());
      std::cout << "W pt=" << W_tmp.Perp() << std::endl;
      float dphi_W_muon = initial_muon.DeltaPhi(W_tmp); 
      float angle_W_muon = vec_initial_muon.Angle(vec_W_tmp); 
      std::cout << "W_muon dphi/angle=" << dphi_W_muon << "/" << angle_W_muon << std::endl;
      float muon_pt_to_W = fabs(initial_muon_p*sin(angle_W_muon));
  ...
  //Trigger matching
  //trig1 = evt.muon()[muid].HLT_mu26_ivarmedium();
  //trig2 = evt.muon()[muid].HLT_mu50();
  //if(!trig1 && !trig2) return;
  bool inimu_trig_match = evt.inimu()[muid].trigger_match();
  std::cout << "trigger_match=" << inimu_trig_match << std::endl;
  if(!inimu_trig_match) return;
  ...
  //MET & MET+MWT 
  TLorentzVector l;
  l.SetPtEtaPhiM(evt.inimu()[muid].pt(),evt.inimu()[muid].eta(),evt.inimu()[muid].phi(),105.6);
  ...
  for (int ljid=0; ljid < evt.largeJet().size(); ++ljid) {
    ...
    float deta = evt.largeJet()[ljid].mom().Eta() - evt.inimu()[muid].eta();
    float dphi = evt.largeJet()[ljid].mom().Phi() - evt.inimu()[muid].phi();
    float dr = sqrt(deta*deta+dphi*dphi);
    //std::cout << "large jet pt/eta/good=" << lj.Perp()*1e-3 << "/" << lj.Eta() << "/" << good << std::endl;
    if(ljetid!=-1 && dr<mindr_ljet_lepton){
      mindr_ljet_lepton = dr;
      closest_ljetid=ljid;
    }
  ...
  for (int ljid=0; ljid < evt.largeJet().size(); ++ljid) {
    ...
    if(fabs(delta_phi_large_jet_lepton)>2.3 && delta_r_large_jet_small_jet>1.5) nGoodLargeJets++;
  }
  std::cout << "nGoodLargeJets=" << nGoodLargeJets << std::endl;
  h->h1D("nGoodLargeJets", "", suffix)->Fill(nGoodLargeJets,weight);
  if(nGoodLargeJets==0) return;
  //if(nGoodLargeJets>1) return;
  ...
  float deta_mu_nu = l.Eta() - nu.Eta();
  float dphi_mu_nu = acos(cos(l.Phi() - nu.Phi()));
  float dr_mu_nu = sqrt(deta_mu_nu*deta_mu_nu + dphi_mu_nu);
  h->h1D(Form("drMuNu%s",channel.c_str()), "", suffix)->Fill(dr_mu_nu, weight);
  
  std::cout << "ljetid def/closest=" << ljetid << "/" << closest_ljetid << std::endl;
  const TLorentzVector &ljt = evt.largeJet()[ljetid].mom();
  const TLorentzVector &closest_ljt = evt.largeJet()[closest_ljetid].mom();
  float mtt = (ljt+sj+nu+l).M();
  h->h2D(Form("mtt_vs_drMuNu%s",channel.c_str()), "", suffix)->Fill(mtt*1e-3,dr_mu_nu, weight);
  float mttChange,mltChange;
  if(!evt.largeJet()[closest_ljetid].good()) mttChange = (ljt+sj+nu+l).M();
  else mttChange = (ljt+closest_ljt+l).M();
  float mttNoNeutrino = (ljt+sj+l).M();
  float mttClosestLjetLepton = (ljt+closest_ljt+l).M();
  float mttClosestLjet = (ljt+closest_ljt).M();
  float mlt = (sj+nu+l).M();
  if(!evt.largeJet()[closest_ljetid].good()) mltChange = (sj+nu+l).M();
  else mltChange = (closest_ljt+l).M();
  float mltClosestLjet = closest_ljt.M();
  float mltClosestLjetLepton = (l+closest_ljt).M();
  float mht = ljt.M();
  h->h1D("mttAll", "", suffix)->Fill(mtt*1e-3, weight);
  h->h1D("mttChangeAll", "", suffix)->Fill(mttChange*1e-3, weight);
  h->h1D(Form("closestLjetGood%s",channel.c_str()), "", suffix)->Fill(evt.largeJet()[closest_ljetid].good(), weight);
  h->h1D(Form("closestLjetGood50%s",channel.c_str()), "", suffix)->Fill(evt.largeJet()[closest_ljetid].good50(), weight);
  h->h1D(Form("mtt%s",channel.c_str()), "", suffix)->Fill(mtt*1e-3, weight);
  h->h1D(Form("closestLjetMass%s",channel.c_str()), "", suffix)->Fill(closest_ljt.M()*1e-3, weight);
  h->h1D(Form("closestSmallJetMass%s",channel.c_str()), "", suffix)->Fill(sj.M()*1e-3, weight);
  h->h1D(Form("mttChange%s",channel.c_str()), "", suffix)->Fill(mttChange*1e-3, weight);
  h->h1D(Form("mttClosestLjet%s",channel.c_str()), "", suffix)->Fill(mttClosestLjet*1e-3, weight);
  h->h1D(Form("mttClosestLjetLepton%s",channel.c_str()), "", suffix)->Fill(mttClosestLjetLepton*1e-3, weight);
  h->h1D(Form("mlt%s",channel.c_str()), "", suffix)->Fill(mlt*1e-3, weight);
  h->h1D(Form("mltChange%s",channel.c_str()), "", suffix)->Fill(mltChange*1e-3, weight);
  h->h1D(Form("mltClosestLjet%s",channel.c_str()), "", suffix)->Fill(mltClosestLjet*1e-3, weight);
  h->h1D(Form("mltClosestLjetLepton%s",channel.c_str()), "", suffix)->Fill(mltClosestLjetLepton*1e-3, weight);
  h->h1D(Form("mht%s",channel.c_str()), "", suffix)->Fill(mht*1e-3, weight);
  h->h2D(Form("mtt_vs_mlt%s",channel.c_str()), "", suffix)->Fill(mtt*1e-3,mlt*1e-3, weight);
  h->h2D(Form("mtt_vs_mht%s",channel.c_str()), "", suffix)->Fill(mtt*1e-3,mht*1e-3, weight);
  h->h2D(Form("mtt_vs_closestSmallJetMass%s",channel.c_str()), "", suffix)->Fill(mtt*1e-3,sj.M()*1e-3, weight);
  h->h1D(Form("mttNoNeutrino%s",channel.c_str()), "", suffix)->Fill(mttNoNeutrino*1e-3, weight);
  h->h1D(Form("mtt%s_%s",type[leading_muon_type].c_str(),channel.c_str()), "", suffix)->Fill(mtt*1e-3, weight);
  h->h1D(Form("mttNoNeutrino%s_%s",type[leading_muon_type].c_str(),channel.c_str()), "", suffix)->Fill(mttNoNeutrino*1e-3, weight);
  if(channel=="SemiLepMu") {
    float deta = evt.inimu()[muid].eta() - semilepmu.Eta(); 
    float dphi = acos(cos(evt.inimu()[muid].phi() - semilepmu.Phi())); 
    float dr = sqrt(deta*deta+dphi*dphi);
    h->h1D("drTruthMuonInitialMuon_final", "", suffix)->Fill(dr, weight);
  }
  if(channel=="FullHad") h->h1D("nGoodLargeJets50", "", suffix)->Fill(nGoodLargeJets50,weight);

  if(!evt.passes("allhad_smooth")) return;
