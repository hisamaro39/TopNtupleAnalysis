...
AnaTtresOR::AnaTtresOR(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
  ...
  m_hSvc.create1D("wpt", "; W p_{T} [GeV] ; Events", 20, 0, 1000);
  m_hSvc.create1D("wpt_passMet", "; W p_{T} [GeV] ; Events", 20, 0, 1000);
...
//    m_hSvc.create1DP(name, "; #Delta R; Events", 31, 0.2, 0.5, 0., 1.);
  ...
  if (m_boosted) {
    m_hSvc.create1D("closeJetPt", "; selected Jet p_{T} [GeV] ; Events", 50, 0, 1000);
    m_hSvc.create1DVar("largeJetPt", "; large jet p_{T} [GeV] ; Events", varBinN2, varBin2);
    m_hSvc.create1DVar("largeJetPt_passMet", "; large jet p_{T} [GeV] ; Events", varBinN2, varBin2);
    m_hSvc.create1DVar("largeJetPt_passLJet", "; large jet p_{T} [GeV] ; Events", varBinN2, varBin2);
...
AnaTtresOR::~AnaTtresOR() {
}

void AnaTtresOR::run(const Event &evt, double weight, const std::string &s, int is2016run) {
  ...
  if (!m_electron && evt.electron().size() != 0)
    return;
 
  if (m_boosted){
    if (is2016run){
      if (!(evt.passes("bejets_2016") || evt.passes("bmujets_nolepton"))) 
        return;
    }else{
  ...
  if (m_electron) {
    ...
    //std::cout << "Number of jet is " << evt.jet().size() << std::endl;
    //2nd muon veto
    if(evt.muon().size()==0) return;
    else if(evt.muon().size()>1) {
      for(int im=1;im<evt.muon().size();im++){
        bool isTight_2nd = evt.muon()[im].isTight();
        //if(!isTight_2nd) continue;
        TLorentzVector m = evt.muon()[im].mom();
        //std::cout << "muon pt=" << m.Perp()*1e-3 << std::endl;
        //OR for 2nd lepton
        float close_deltaR_jl = 99;
        for (int jet_id=0; jet_id < evt.jet().size(); ++jet_id){    
          float dphi = m.DeltaPhi(evt.jet()[jet_id].mom());
          float dy = m.Rapidity() - evt.jet()[jet_id].mom().Rapidity();
          float deltaR_tmp = std::sqrt(dy*dy + dphi*dphi);
          //std::cout << "deltaR=" << deltaR_tmp << std::endl;
          if (deltaR_tmp < close_deltaR_jl) close_deltaR_jl = deltaR_tmp;
        }
        //std::cout << "close_deltaR_jl=" << close_deltaR_jl << std::endl;
        if( close_deltaR_jl > 0.04 + 10./(m.Perp()*1e-3) ) {
          if(m.Perp()*1e-3>25) return;
        }
      }
    }
    ...
    //std::cout << "isTight 1st/2nd=" << isTight << "/" << isTight2 << std::endl;
    if(!isTight) return;
  ...
  //closest small jet & lepton cut
  float close_deltaR_jl = 99;
  for (int jet_id=0; jet_id < evt.jet().size(); ++jet_id){    
    //if(evt.jet()[jet_id].closeToLepton()){
      //std::cout << "id=" << jet_id << std::endl;
      //std::cout << "close to lepton is " <<  evt.jet()[jet_id].closeToLepton() << std::endl;
      float dphi = l.DeltaPhi(evt.jet()[jet_id].mom());
      float dy = l.Rapidity() - evt.jet()[jet_id].mom().Rapidity();
      float deltaR_tmp = std::sqrt(dy*dy + dphi*dphi);
      //std::cout << "deltaR=" << deltaR_tmp << std::endl;
      //if(deltaR_tmp<close_deltaR_jl) close_deltaR_jl=deltaR_tmp;
    //}
  ...
  //std::cout << "close_deltaR_jl=" << close_deltaR_jl << std::endl;
  if( close_deltaR_jl < 0.04 + 10./(l.Perp()*1e-3) ) return;
  if(close_deltaR_jl>1.5) return;
  ...
  for (; ljet_idx < evt.largeJet().size(); ++ljet_idx) {
    ...
    if(fabs(delta_phi_large_jet_lepton)>2.3 && good_def) break;
  }
  ...
  h->h2D("lepPt_vs_drMuonJet", "", suffix)->Fill(l.Perp()*1e-3,closejl_deltaR, weight);

  h->h1D("trueMtt", "", suffix)->Fill(evt.MC_ttbar_beforeFSR().M()*1e-3, weight);
  _tree_truemtt = evt.MC_ttbar_beforeFSR().M()*1e-3;

  if (m_boosted && (is2016run && (evt.passes("bejets_2016") || evt.passes("bmujets_nolepton")))
      || (!is2016run && (evt.passes("bejets_2015") || evt.passes("bmujets_2015")))) {

    for (int ij=0; ij < evt.jet().size(); ++ij){
      TLorentzVector smalljet = evt.jet()[ij].mom();
      float jet_eta = smalljet.Eta();
      float jet_phi = smalljet.Phi();
      float lep_eta = l.Eta(); 
      float lep_phi = l.Phi(); 
      float deta = jet_eta-lep_eta;
      float dphi = acos(cos(jet_phi-lep_phi));
      float dr_muon_jet = sqrt(deta*deta+dphi*dphi);
      float lep_pt = l.Perp()*1e-3;
      //h->h2D("lepPt_vs_drMuonJet", "", suffix)->Fill(lep_pt,dr_muon_jet, weight);
    }
    ...
    //close_idx=0;//temporal
    //std::cout << "close_idx=" << close_idx << std::endl;
    ...
    //std::cout << "godljet idx=" << goodljet_idx << std::endl;
    ...
    if (((is2016run && evt.passes("bmujets_nolepton") || !is2016run && evt.passes("bmujets_2015"))) && m_boosted && !m_electron) _tree_cat = 1;
    _tree_syst = s;
    h->m_tree->Fill();
