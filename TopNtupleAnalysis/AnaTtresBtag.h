 ...
 */
#ifndef ANATTRESBTAG_H
#define ANATTRESBTAG_H
...
class AnaTtresBtag : public AnalysisMVATopTag {
  public:
    AnaTtresBtag(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList);
    virtual ~AnaTtresBtag();

    void run(const Event &e, double weight, const std::string &syst, int is2016runm, float drlj_thr);
    void terminate() {};
