...
#ifndef ANALYSISVALIDATION_H
#define ANALYSISVALIDATION_H
...
class AnalysisValidation {
  public:
    AnalysisValidation(const std::string &filename, std::vector<std::string> &systematicList);
    AnalysisValidation(const std::string &filename);
    virtual ~AnalysisValidation();

    virtual void run(const Event &e, double weight, const std::string &systUnc = "", int mode=0, TSpline *m_spline=nullptr) = 0;
