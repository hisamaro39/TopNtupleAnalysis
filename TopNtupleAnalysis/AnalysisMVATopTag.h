...
#ifndef ANALYSISMVATOPTAG_H
#define ANALYSISMVATOPTAG_H
...
class AnalysisMVATopTag {
  public:
    AnalysisMVATopTag(const std::string &filename, std::vector<std::string> &systematicList);
    AnalysisMVATopTag(const std::string &filename);
    virtual ~AnalysisMVATopTag();

    virtual void run(const Event &e, double weight, const std::string &systUnc = "", int is2016run=0, float bdtthr=0) = 0;
