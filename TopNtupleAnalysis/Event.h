...
class Event {
  public:
    ...
    std::vector<IniMuon> &inimu();
    std::vector<Jet> &jet();
    std::vector<Jet> &truthjet();
    std::vector<LargeJet> &largeJet();
    std::vector<LargeJet> &truthlargeJet();

    std::vector<Jet> &tjet();
    std::vector<Jet> &nocalibtjet();
    std::vector<Jet> &vrtjet();
    std::vector<Jet> &nocalibjet();
    std::vector<Jet> &nocalibljet();
    std::vector<Muon> &nocalibmu();
    std::vector<Electron> &nocalibel();
    ...
    const std::vector<Truth> &truth() const;
    const std::vector<IniMuon> &inimu() const;
    const std::vector<Jet> &jet() const;
    const std::vector<Jet> &truthjet() const;
    const std::vector<LargeJet> &largeJet() const;
    const std::vector<LargeJet> &truthlargeJet() const;

    const std::vector<Jet> &tjet() const;
    const std::vector<Jet> &nocalibtjet() const;
    const std::vector<Jet> &vrtjet() const;
    const std::vector<Jet> &nocalibjet() const;
    const std::vector<Jet> &nocalibljet() const;
    const std::vector<Muon> &nocalibmu() const;
    const std::vector<Electron> &nocalibel() const;
    ...
    float &MC_met();
    const float MC_met() const;

    float &chi2_all();
    const float chi2_all() const;

    float &MC_met_nomuon();
    const float MC_met_nomuon() const;

    float &MC_met_muon();
    const float MC_met_muon() const;
    ...
    TLorentzVector &MC_th();
    const TLorentzVector &MC_th() const;

    TLorentzVector &MC_tl();
    const TLorentzVector &MC_tl() const;
  ...
  protected:
    ...
    std::vector<Truth> m_truth;
    std::vector<IniMuon> m_inimu;
    std::vector<Jet> m_jet;
    std::vector<Jet> m_truthjet;
    std::vector<LargeJet> m_truthlargeJet;
    std::vector<LargeJet> m_largeJet;

    TLorentzVector m_met;

    std::vector<Jet> m_tjet;
    std::vector<Jet> m_nocalibtjet;
    std::vector<Jet> m_vrtjet;
    std::vector<Jet> m_nocalibjet;
    std::vector<Jet> m_nocalibljet;
    std::vector<Muon> m_nocalibmu;
    std::vector<Electron> m_nocalibel;
    ...
    TLorentzVector m_MC_th;
    TLorentzVector m_MC_tl;
