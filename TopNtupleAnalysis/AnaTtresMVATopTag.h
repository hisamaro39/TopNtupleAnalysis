 ...
 */
#ifndef ANATTRESMVATOPTAG_H
#define ANATTRESMVATOPTAG_H
...
class AnaTtresMVATopTag : public AnalysisMVATopTag {
  public:
    AnaTtresMVATopTag(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList);
    virtual ~AnaTtresMVATopTag();

    void run(const Event &e, double weight, const std::string &syst, int is2016run, float bdtthr);
    void terminate() {};
