...
// Process: g u~ > t t~ u~ WEIGHTED<=3 @2
// Process: g c~ > t t~ c~ WEIGHTED<=3 @2
// Process: g d~ > t t~ d~ WEIGHTED<=3 @2
// Process: g s~ > t t~ s~ WEIGHTED<=3 @2
...
void CPPProcess::calculate_wavefunctions(const int perm[], const int hel[])
{
  ...
  FFV1_0(w[4], w[10], w[6], pars->GC_11, amp[3]); 
...
double CPPProcess::matrix_2_gux_ttxux() 
{
  ...
  jamp[0] = +1./2. * (+amp[2] + amp[3] + std::complex<double> (0, 1) * amp[4]); 
  jamp[1] = +1./2. * (-1./3. * amp[0] - 1./3. * amp[3]); 
  jamp[2] = +1./2. * (-1./3. * amp[1] - 1./3. * amp[2]); 
  jamp[3] = +1./2. * (+amp[0] + amp[1] - std::complex<double> (0, 1) * amp[4]); 
