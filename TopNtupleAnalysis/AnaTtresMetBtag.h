 ...
 */
#ifndef ANATTRESMETBTAG_H
#define ANATTRESMETBTAG_H
...
class AnaTtresMetBtag : public AnalysisValidation {
  public:
    AnaTtresMetBtag(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList);
    virtual ~AnaTtresMetBtag();

    //void run(const Event &e, double weight, const std::string &syst, int is2016run, float totalEvents);
    void run(const Event &e, double weight, const std::string &syst, int mode, TSpline *m_spline);
    void terminate() {};
