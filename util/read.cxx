...
#include "TopNtupleAnalysis/AnaTtresNoIso.h"
#include "TopNtupleAnalysis/AnaTtres21.h"
#include "TopNtupleAnalysis/AnaTtres21ResoMu.h"
#include "TopNtupleAnalysis/AnaTtres21ResoEl.h"
#include "TopNtupleAnalysis/AnaTtres21_electron.h"
#include "TopNtupleAnalysis/AnaTtresFullHad.h"
#include "TopNtupleAnalysis/AnaTtresTruthResolved.h"
#include "TopNtupleAnalysis/AnaTtresOR.h"
#include "TopNtupleAnalysis/AnaTtresMetTrig.h"
#include "TopNtupleAnalysis/AnaTtresSLMtt.h"
#include "TopNtupleAnalysis/AnaTtresQCD.h"
#include "TopNtupleAnalysis/AnaTtresMM.h"
#include "TopNtupleAnalysis/AnaTtresTrigMuonMetWpt.h"
#include "TopNtupleAnalysis/AnaTtresSLWpt.h"
#include "TopNtupleAnalysis/AnaTtresTrigMuonMetNoWpt.h"
#include "TopNtupleAnalysis/AnaTtresTrigMuonLjetLjetpt.h"
#include "TopNtupleAnalysis/AnaTtresTrigMuonLjetNoLjetpt.h"
#include "TopNtupleAnalysis/AnaTtresTrigMetOnly.h"
#include "TopNtupleAnalysis/AnaTtresTrigLjetOnly.h"
#include "TopNtupleAnalysis/AnaTtresNoTrigger.h"
...
double wjetsSF(const Event &sel, const std::string &syst) {
  ...
  //const unsigned int runNumber = (isData)? sel.eventNumber() : sel.randomRunNumber();
...
int main(int argc, char **argv) {
  ...
  static struct extendedOption extOpt[] = {
        ...
        {"mode",                  required_argument,     0, 'r', "Is this 2016 run?", &mode, extendedOption::eOTInt},
        {"boost",                  required_argument,     0, 'b', "Boosted channel only", &onlyBoosted, extendedOption::eOTInt},
        {"trthr",                  required_argument,     0, 'p', "W or ljet pt threshold for MET or ljet trigger", &trthr, extendedOption::eOTInt},
        {"bdtthr",                  required_argument,     0, 'C', "Threshold for BDT top tagging", &bdtthr, extendedOption::eOTInt},
        {"drlj",                  required_argument,     0, 'F', "Threshold for dr of closest lepton and jet", &drlj, extendedOption::eOTInt},
        {"bwp",                  required_argument,     0, 'G', "B-tagging working point", &bwp, extendedOption::eOTInt},
        {"debug",                  required_argument,     0, 'k', "Debug mode", &Debug, extendedOption::eOTInt},
  ...
  //setup hyb b-tag tool
  std::cout << "setup hyb b-tag tool" << std::endl;
  TFile *m_inf = TFile::Open("/cvmfs/atlas.cern.ch/repo/sw/database/GroupData/xAODBTaggingEfficiency/13TeV/2017-21-13TeV-MC16-CDI-2017-07-02_v1.root", "read");
  TSpline *m_spline = (TSpline3*) m_inf->Get("MV2c10/AntiKt4EMTopoJets/HybBEff_70/cutprofile");
  //TFile *m_inf = TFile::Open("./share/cutprofile_MV2c10_AntiKet4EMTopoJets_HybBEff_70.root", "read");//made by myself
  //TSpline *m_spline = (TSpline3*) m_inf->Get("cutprofile");
  ...
  std::cout << "sumWeightsFile=" << sumWeightsFile << std::endl;
  if(!isData && sumWeightsFile == "") {
    std::cout << "!isData && no sumWeightsFile" << std::endl;
    ...
    for (int k = 0; k < t_sumWeights.GetEntries(); ++k) {
      ...
      std::cout << "dsid/value/sumOfWeights=" << dsid << "/" << value << "/" << sumOfWeights[dsid] << std::endl;
    }
  ...
  std::vector<AnalysisTrig *> vec_analysis_trig;
  std::vector<AnalysisMVATopTag *> vec_analysis_mva_top_tag;
  std::vector<AnalysisValidation *> vec_analysis_validation;
  
  if (analysis == "AnaTtresSL") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresSL(outList[0], true,  false, systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresSL(outList[1], false, false, systsListWithBlankNominal)); // resolved muon    
    if(onlyBoosted) vec_analysis.push_back(new AnaTtresSL(outList[2], true,  true,  systsListWithBlankNominal)); // boosted  electron
    if(onlyBoosted) vec_analysis.push_back(new AnaTtresSL(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  if (analysis == "AnaTtresHtaumu") {
    if(onlyBoosted) vec_analysis.push_back(new AnaTtresHtaumu(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresTruth") {
    vec_analysis.push_back(new AnaTtresTruth(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresTruthElectron") {
    vec_analysis.push_back(new AnaTtresTruthElectron(outList[2], true,  true,  systsListWithBlankNominal)); // boosted  electron
  }
  else if (analysis == "AnaTtresVRBtag") {
    vec_analysis.push_back(new AnaTtresVRBtag(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresNoIso") {
    vec_analysis.push_back(new AnaTtresNoIso(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtres21") {
    vec_analysis.push_back(new AnaTtres21(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtres21_electron") {
    vec_analysis.push_back(new AnaTtres21_electron(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
  }
  else if (analysis == "AnaTtres21ResoMu") {
    vec_analysis.push_back(new AnaTtres21ResoMu(outList[1], false, false,  systsListWithBlankNominal)); // resolved muon
  }
  else if (analysis == "AnaTtres21ResoEl") {
    vec_analysis.push_back(new AnaTtres21ResoEl(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
  }
  else if (analysis == "AnaTtresFullHad") {
    vec_analysis.push_back(new AnaTtresFullHad(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresTruthResolved") {
    //if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTruthResolved(outList[0], true,  false, systsListWithBlankNominal)); // resolved electron
    vec_analysis.push_back(new AnaTtresTruthResolved(outList[1], false, false, systsListWithBlankNominal)); // resolved muon    
    //vec_analysis.push_back(new AnaTtresTruthResolved(outList[2], true,  true,  systsListWithBlankNominal)); // boosted  electron
    //vec_analysis.push_back(new AnaTtresTruthResolved(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresOR") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresOR(outList[0], true,  false, systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresOR(outList[1], false, false, systsListWithBlankNominal)); // resolved muon    
    vec_analysis.push_back(new AnaTtresOR(outList[2], true,  true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresOR(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresMetTrig") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresMetTrig(outList[0], true,  false, systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresMetTrig(outList[1], false, false, systsListWithBlankNominal)); // resolved muon    
    vec_analysis.push_back(new AnaTtresMetTrig(outList[2], true,  true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresMetTrig(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresTrigMuonMetWpt") {
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresTrigMuonMetWpt(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresTrigMuonMetWpt(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_trig.push_back(new AnaTtresTrigMuonMetWpt(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_trig.push_back(new AnaTtresTrigMuonMetWpt(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresTrigMuonLjetLjetpt") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonLjetLjetpt(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonLjetLjetpt(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis.push_back(new AnaTtresTrigMuonLjetLjetpt(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresTrigMuonLjetLjetpt(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresTrigMuonMetNoWpt") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonMetNoWpt(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonMetNoWpt(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis.push_back(new AnaTtresTrigMuonMetNoWpt(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresTrigMuonMetNoWpt(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresTrigMuonLjetNoLjetpt") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonLjetNoLjetpt(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigMuonLjetNoLjetpt(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis.push_back(new AnaTtresTrigMuonLjetNoLjetpt(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresTrigMuonLjetNoLjetpt(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresTrigMetOnly") {
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresTrigMetOnly(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresTrigMetOnly(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_trig.push_back(new AnaTtresTrigMetOnly(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_trig.push_back(new AnaTtresTrigMetOnly(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresTrigLjetOnly") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigLjetOnly(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresTrigLjetOnly(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis.push_back(new AnaTtresTrigLjetOnly(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresTrigLjetOnly(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresNoTrigger") {
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresNoTrigger(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis.push_back(new AnaTtresNoTrigger(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis.push_back(new AnaTtresNoTrigger(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis.push_back(new AnaTtresNoTrigger(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresSLWpt") {
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresSLWpt(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis_trig.push_back(new AnaTtresSLWpt(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_trig.push_back(new AnaTtresSLWpt(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_trig.push_back(new AnaTtresSLWpt(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresBtag") {
    if(!onlyBoosted) vec_analysis_mva_top_tag.push_back(new AnaTtresBtag(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis_mva_top_tag.push_back(new AnaTtresBtag(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_mva_top_tag.push_back(new AnaTtresBtag(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_mva_top_tag.push_back(new AnaTtresBtag(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  ...
  else if (analysis == "AnaTtresMVATopTag") {
    if(!onlyBoosted) vec_analysis_mva_top_tag.push_back(new AnaTtresMVATopTag(outList[0], true, false,  systsListWithBlankNominal)); // resolved electron
    if(!onlyBoosted) vec_analysis_mva_top_tag.push_back(new AnaTtresMVATopTag(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_mva_top_tag.push_back(new AnaTtresMVATopTag(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_mva_top_tag.push_back(new AnaTtresMVATopTag(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  } 
  else if (analysis == "AnaTtresMetBtag") {
    vec_analysis_validation.push_back(new AnaTtresMetBtag(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_validation.push_back(new AnaTtresMetBtag(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresValidation") {
    vec_analysis_validation.push_back(new AnaTtresValidation(outList[0], true, false,  systsListWithBlankNominal)); // resolved  electron
    vec_analysis_validation.push_back(new AnaTtresValidation(outList[1], false, false,  systsListWithBlankNominal)); // resolved  muon
    vec_analysis_validation.push_back(new AnaTtresValidation(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_validation.push_back(new AnaTtresValidation(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresMultiJets") {
    vec_analysis_validation.push_back(new AnaTtresMultiJets(outList[2], true, true,  systsListWithBlankNominal)); // boosted  electron
    vec_analysis_validation.push_back(new AnaTtresMultiJets(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  else if (analysis == "AnaTtresCheckTrigger") {
    vec_analysis_validation.push_back(new AnaTtresCheckTrigger(outList[3], false, true,  systsListWithBlankNominal)); // boosted  muon
  }
  ...
  //MiniTree mttr(false, fileList[0], "truth");
  ...
  for (size_t systIdx = 0; systIdx < systsList.size(); ++systIdx) {
    ...
    for (size_t lmodeIdx = 0; lmodeIdx < lepton_modes.size(); ++lmodeIdx) {
      ...
      Event sel,seltr; // selected objects
      ...
      for (int k = 0; k < nentries; ++k) {
        ...
        }else std::cout << "("<< tname << ") Entry " << k << "/" << nentries << std::endl;

        bool isTight = false;
    
        // read the event into an Event object to be sent to the Analysis code later

        //std::cout << "read sel" << std::endl;
        mt.read(k, sel);
        //std::cout << "read seltr" << std::endl;
        //mttr.read(k, seltr);
	...
	const unsigned int runNumber = sel.runNumber_or_RandomRunNumber();
	//const unsigned int runNumber = (isData)? sel.runNumber() : sel.randomRunNumber();
	//std::cout << "isData/runNumber=" << isData << "/" << sel.runNumber() << std::endl;
        ...
        for (size_t wIdx = 0; wIdx < weightSystematics.size(); ++wIdx) {
          ...
          double weight = 1;
          if (!isData) {
            if(analysis=="AnaTtresMetTrig") weight *= sel.weight_mc();	    	    
            else weight *= sel.weight_mc()*sel.weight_pileup();	    	    
            weight *= sampleXsection.getXsection(channel);
            //std::cout << "weight mc/pileup/xsec=" << sel.weight_mc() << "/" << sel.weight_pileup() << "/" << sampleXsection.getXsection(channel) << std::endl;
			...
			btagsf *= mt.vf(pref_pt)->at(bidx);
                ...
                //btagsf = mt.f(pref);
                if(mt.vf("tjet_mv2c10") && mt.vi("tjet_numConstituents")){
                for (size_t bidx = 0; bidx < mt.vf("tjet_mv2c10")->size(); ++bidx) {
                  TLorentzVector pb;
                  pb.SetPtEtaPhiE(mt.vf("tjet_pt")->at(bidx), mt.vf("tjet_eta")->at(bidx), mt.vf("tjet_phi")->at(bidx), mt.vf("tjet_e")->at(bidx));
                  if (mt.vf("tjet_pt")->at(bidx) > 10e3 &&
          ...
          float b_bdt = -9999;
          if(bwp==70) b_bdt = 0.6455;
          else if(bwp==77) b_bdt = 0.3706;
          else if(bwp==85) b_bdt = -0.1416;
          else if(bwp==100) b_bdt = -9999;//temporal value
          else {
            cout << "invalid b-tag WP " << bwp << endl;
            return 0;
          }
          ...
          if (_btags > 0) { //calo b-tagging
            ...
            for (size_t bidx = 0; bidx < mt.vf("tjet_mv2c10")->size(); ++bidx) {
              TLorentzVector pb;
              pb.SetPtEtaPhiE(mt.vf("tjet_pt")->at(bidx), mt.vf("tjet_eta")->at(bidx), mt.vf("tjet_phi")->at(bidx), mt.vf("tjet_e")->at(bidx));
              if (mt.vf("tjet_mv2c10")->at(bidx) > b_bdt && mt.vf("tjet_pt")->at(bidx) > 10e3 && 
                  std::fabs(mt.vf("tjet_eta")->at(bidx)) < 2.5 && mt.vi("tjet_numConstituents")->at(bidx) >= 2) {
                nBtagged += 1;
              }
            }
            if (mt.vf("tjet_pt")->size() > 0) {
              if (mt.vf("tjet_mv2c10")->at(0) > b_bdt && mt.vf("tjet_pt")->at(0) > 10e3 &&
	  ...
	  if (analysis=="AnaTtresSL") { // signal region
	    for (size_t iAna = 0; iAna < vec_analysis.size(); ++iAna) {
		   ...
		   if(nBtagged==0){		     
		     ...
		     if(iselect)	weight = MM_1b_re->getMMweights(sel, runMM_StatErr, iselect, 0, runNumber);// use w+jets rates (>=2 jets) everywhere for the time being
                ...
                vec_analysis[iAna]->run(sel, weight, suffix, mode);
	    }
          } 
          else if (analysis=="AnaTtresHtaumu") { 
	    for (size_t iAna = 0; iAna < vec_analysis.size(); ++iAna) {
              vec_analysis[iAna]->run(sel, weight, suffix, mode);
            }
          } 
	    ...
	    for (size_t iAna = 0; iAna < vec_analysis_trig.size(); ++iAna) {
              vec_analysis_trig[iAna]->run(sel, weight, suffix, mode, trthr);
            }
          } 
          else if (analysis=="AnaTtresTrigMuonLjetLjetpt") { 
	    for (size_t iAna = 0; iAna < vec_analysis.size(); ++iAna) {
	    ...
	    for (size_t iAna = 0; iAna < vec_analysis_trig.size(); ++iAna) {
            ...
            float drlj_thr = drlj*0.1;
	    for (size_t iAna = 0; iAna < vec_analysis_mva_top_tag.size(); ++iAna) {
              vec_analysis_mva_top_tag[iAna]->run(sel, weight, suffix, mode, drlj_thr);
            }
          } 
          else if (analysis=="AnaTtresMVATopTag") { 
            float applied_thr = bdtthr*0.01;
	    for (size_t iAna = 0; iAna < vec_analysis_mva_top_tag.size(); ++iAna) {
              vec_analysis_mva_top_tag[iAna]->run(sel, weight, suffix, mode, applied_thr);
            }
          } 
          else if (analysis=="AnaTtresMetBtag") { 
	    for (size_t iAna = 0; iAna < vec_analysis_validation.size(); ++iAna) {
              vec_analysis_validation[iAna]->run(sel, weight, suffix, mode,m_spline);
            }
          } 
          else if (analysis=="AnaTtresValidation") { 
            weight = sampleXsection.getXsection(channel);
            ...
            weight /= sumOfWeights[channel];
            //std::cout << "channel/sumOfWeights=" << channel << "/" << sumOfWeights[channel] << std::endl;
            //std::cout << "weight mc/pileup=" << sel.weight_mc() << "/" << sel.weight_pileup() << std::endl;
	    for (size_t iAna = 0; iAna < vec_analysis_validation.size(); ++iAna) {
              vec_analysis_validation[iAna]->run(sel,weight, suffix, mode,m_spline);
            }
          } 
            ...
            for (size_t iAna = 0; iAna < vec_analysis_validation.size(); ++iAna) {
              bool iselect((dynamic_cast<AnaTtresMultiJets*>(vec_analysis_validation[iAna]))->isElectron());
              std::cout << "iselect=" << iselect << std::endl;
  ...
  for (size_t iAna = 0; iAna < vec_analysis_trig.size(); ++iAna) {
    std::cout << "Removed " << vec_analysis_trig[iAna]->getNduplicate() << " duplicate entries" << std::endl;
    vec_analysis_trig[iAna]->terminate();
    delete vec_analysis_trig[iAna];
  }
  for (size_t iAna = 0; iAna < vec_analysis_mva_top_tag.size(); ++iAna) {
    std::cout << "Removed " << vec_analysis_mva_top_tag[iAna]->getNduplicate() << " duplicate entries" << std::endl;
    vec_analysis_mva_top_tag[iAna]->terminate();
    delete vec_analysis_mva_top_tag[iAna];
  }
  for (size_t iAna = 0; iAna < vec_analysis_validation.size(); ++iAna) {
    std::cout << "Removed " << vec_analysis_validation[iAna]->getNduplicate() << " duplicate entries" << std::endl;
    vec_analysis_validation[iAna]->terminate();
    delete vec_analysis_validation[iAna];
  }


  vec_analysis.clear();
  vec_analysis_trig.clear();
  vec_analysis_mva_top_tag.clear();
  vec_analysis_validation.clear();
