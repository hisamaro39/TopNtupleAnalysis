...
void loadConfig(const std::string &file) {
  ...
  std::cout << "config file=" << file << std::endl;
...
void drawCompare(SampleSetConfiguration &stackConfig, const vector<std::string> &extraText, const std::string &outfile, bool ratio, double lumi) {
  ...
  //string _stampText = "work in progress";
  if (_stamp == 1)
...
void drawDataMC(SampleSetConfiguration &stackConfig, const vector<std::string> &extraText, const std::string &outfile, bool ratio, const std::string &xTitle, const std::string &yTitle, bool mustBeBigger, int posLegend, float yMin, float yMax, int useArrow, double lumi) {
  std::cout << "util::drawDataMC" << std::endl;
  for(int t=0;t<extraText.size();t++){
    std::cout << "extraText=" << extraText[t] << std::endl;
  }
  std::cout << "xTitle/yTitle=" << xTitle << "/" << yTitle << std::endl;
  TStyle *atlasStyle = AtlasStyle();
  gROOT->SetStyle("ATLAS");
  gROOT->ForceStyle();
  ...
  if (ratio && Data) {
    ...
    if (xTitle == "")
      ...
      rat->GetXaxis()->SetTitle(xTitle.c_str());
    std::cout << "setting of histgram" << std::endl;
    ...
    rat->GetYaxis()->SetLabelOffset(0.02);
    rat->GetXaxis()->SetTitleOffset(1.1);
    rat->GetYaxis()->SetLabelSize(0.1);
    rat->GetXaxis()->SetLabelSize(0.13);
    rat->GetYaxis()->SetTitleSize(0.13);
  ...
  //TFile *file_tmp = TFile::Open("/home/mtanaka/WorkSpace/ttbar_analysis/AnalysisNtuple_2016/TopNtupleAnalysis/result_trigger_btag/calo_btag/allbkg.root");
  TFile *file_tmp = TFile::Open("/home/mtanaka/WorkSpace/ttbar_analysis/AnalysisNtuple_2016/TopNtupleAnalysis/result_validation/rel20_smoothed_toptag/allbkg.root");
  TH1 *hist_tmp;
  if(extraText.size()){
    std::cout << "extraText=" << extraText[0] << std::endl;
    if(extraText[0]=="#mu+jets") hist_tmp = (TH1*) file_tmp->Get("xboosted_mu");
    else if(extraText[0]=="e+jets") hist_tmp = (TH1*) file_tmp->Get("xboosted_e");
    std::cout << "hist_tmp=" << hist_tmp << std::endl;
    hist_tmp->SetLineStyle(2);
    hist_tmp->SetMarkerSize(0);
    hist_tmp->SetLineWidth(3);
    hist_tmp->SetLineColor(2);
    hist_tmp->Draw("same");
  }
...
SampleSetConfiguration makeConfigurationPlots(const string &prefix, const string &channel, bool isMcOnly) {
  std::cout << "makeConfigurationPlots" << std::endl;
  ...
  for (std::map<std::string, std::string>::iterator it = name.begin(); it != name.end(); ++it) {
    ...
    std::cout << "id/file/longTitle/latexTitle=" << id << "/" << file << "/" << longTitle << "/" << latexTitle << std::endl;
...
void addAllSystematics(SystematicCalculator &systCalc, const std::string &pref, const std::string &channel, bool updw) {
  std::cout << "addAllSystematics" << std::endl;
  ...
  for (std::map<std::string, std::vector<std::string> >::iterator it = syst.begin(); it != syst.end(); ++it) {
    ...
    std::cout << "up/dw/updw=" << up << "/" << dw << "/" << updw << std::endl;
    if (!updw) {
      if (dw != "-") {
