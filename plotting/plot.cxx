...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::cout << "size of h_items is " << h_items.size() << std::endl;
    if (h_items.size() == 1) 
      histogram = h_items[0];
    else {
    ...
    if (!isRatio) {
      std::cout << "for Data/MC comparison" << std::endl;
      // for Data/MC comparison
      SampleSetConfiguration stackConfig = makeConfigurationPlots(prefix, channel, mcOnly);
      std::cout << "prefix/channel/mcOnly=" << prefix << "/" << channel << "/" << mcOnly << std::endl;
      ...
      std::cout << "xmax/xmin/rebin/normBinWidth=" << xMax << "/" << xMin << "/" << rebin << "/" << normBinWidth << std::endl;
      ...
      std::cout << "if it is a ratio plot" << std::endl;
      ...
      if (!mcOnly) {
        std::cout << "for data eff. numerator" << std::endl;
      ...
      std::cout << "for MC eff. numeraotr using PP" << std::endl;
      ...
      std::cout << "for MC eff. dominator" << std::endl;
      ...
      // calculate ratio
      std::cout << "calculate ratio" << std::endl;
